...
func CompareXlsXlsx(xlsfilepathname string, xlsxfilepathname string) string {
	...
	for sheet, xlsxSheet := range xlsxFile.Sheets {
		...
		for row := 0; row<xlsxSheet.MaxRow; row ++ {
			xlsRow := xlsSheet.Row(row)
			xlsxRow, err := xlsxSheet.Row(row)
                        if(err!=nil){ return fmt.Sprintf("Error reading xlsx row %d: %s",row,err); };
			for cell := 0; cell<xlsxSheet.MaxCol; cell ++ {
				xlsxCell := xlsxRow.GetCell(cell);
                                var xlsxText string;
                                if(xlsxCell.Type()==xlsx.CellTypeNumeric){
				  xlsxText, err = xlsxCell.GeneralNumeric()
                                  if(err!=nil){ return fmt.Sprintf("Error reading xlsx cell %d (row %d): %s",cell,row,err); };
                                }else{     
